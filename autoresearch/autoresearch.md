...
2. **`string.match` for combined whitespace+structural tokens in PUC Lua decode**
...
3. **Benchmark methodology: run N≥5 trials and use median**
...
1. **Encode boolean as direct table lookup**: `BOOLS[val]` instead of `val and "true" or "false"`. Marginal.

2. **Small String Interning Cache for Object Keys**: Tried in Run 8/9, discarded. Overhead > savings.

3. **Extended SMALL_INTS to 0-999**: Tried (archived Run 7), lookup cost > savings. The digit-extraction approach (Tier 2 #5) may be better.
